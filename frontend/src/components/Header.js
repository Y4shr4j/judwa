  ...
  return (
    <header className='h-16 shadow-md bg-white fixed w-full z-40'>
      <div className=' h-full container mx-auto flex items-center px-4 justify-between'>
            <div className=''>
                <Link to={"/"}>
                  <img src="https://dwc6ndmngc4q3.cloudfront.net/20240620155323/assets/images/logo.png" alt="Brand Logo" className='w-32 p-4' />
                </Link>
            </div>
            ...
            <div className='flex items-center gap-7'>
                    ...
                    menuDisplay && (
                      ...
                      }
                      {
                        user?.role === ROLE.GENERAL && (
                          <Link to={"/user-panel/my-orders"} className='whitespace-nowrap hidden md:block hover:bg-slate-100 p-2' onClick={() => setMenuDisplay(prev => !prev)}>User Panel</Link>
                        )
                      }
                    </nav>
                      </div>
                ...
                <div>
                  {
                    user?._id  ? (
                      <button onClick={handleLogout} className='px-3 py-1 text-white bg-gray-600 hover:bg-gray-700'>Logout</button>
                    )
                    : (
                    <Link to={"/login"} className='px-3 py-1  text-white bg-gray-600 hover:bg-gray-700'>Login</Link>
                    )
                  }
